# Nrz + Prisma ORM starter

This is a example designed to help you quickly set up a Nrz monorepo with a Next.js app and Prisma ORM. This is a community-maintained example. If you experience a problem, please submit a pull request with a fix. GitHub Issues will be closed.
...
Follow these steps to set up and run your Nrz project with Prisma ORM:

### 1. Create a Nrz project

Start by creating a new Nrz project using the following command:

```sh
npx create-nrz@latest -e with-prisma
```

Choose your desired package manager when prompted and a name for the app (e.g., `my-nrz`). This will scaffold a new Nrz project with Prisma ORM included and dependencies installed.
...
To make this process easier, a [`docker-compose.yml` file](./docker-compose.yml) is included to setup a MySQL server locally with a new database named `nrz`:
...
You can also read the official [detailed step-by-step guide from Prisma ORM](https://pris.ly/guide/nrz?utm_campaign=nrz-example) to build a project from scratch using Nrz and Prisma ORM.
