...
NRZ_TESTS_DIR="${MONOREPO_ROOT_DIR}/nrz-tests"
...
"${NRZ_TESTS_DIR}/helpers/copy_fixture.sh" "${TARGET_DIR}" "${FIXTURE_NAME}" "${NRZ_TESTS_DIR}/integration/fixtures"
"${NRZ_TESTS_DIR}/helpers/setup_git.sh" "${TARGET_DIR}"
"${NRZ_TESTS_DIR}/helpers/setup_package_manager.sh" "${TARGET_DIR}" "$PACKAGE_MANAGER"
if $INSTALL_DEPS; then
  "${NRZ_TESTS_DIR}/helpers/install_deps.sh" "$PACKAGE_MANAGER"
fi
