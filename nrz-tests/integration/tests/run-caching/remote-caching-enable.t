...
Test that remote caching is enabled by default
  $ ${NRZ} run build --team=khulnasoft --token=hi --output-logs=none 2>/dev/null | grep "Remote caching"
...
Test that remote caching is still enabled
  $ ${NRZ} run build --team=khulnasoft --token=hi --output-logs=none | grep "Remote caching"
  \xe2\x80\xa2 Remote caching enabled (esc)
