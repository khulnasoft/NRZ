...
Run build with package task in non-root nrz.json
  ...
  Error: unnecessary_package_task_syntax (https://nrz.build/messages/unnecessary-package-task-syntax)
...
Run build with invalid env var
  ...
  [1]
  $ sed  's/\[\([^]]*\)\]/\(\1)/g' < error.txt
  invalid_env_prefix (https://nrz.build/messages/invalid-env-prefix)
...
Run in single package mode even though we have a task with package syntax
  ...
  [1]
  $ sed  's/\[\([^]]*\)\]/\(\1)/g' < error.txt
  package_task_in_single_package_mode (https://nrz.build/messages/package-task-in-single-package-mode)
...
Run build with syntax errors in nrz.json
  ...
  Error:   x Expected a property but instead found ','.
     ,-[nrz.json:1:1]
   1 | {
   2 |   "$schema": "https://nrz.build/schema.json",,
     :                                                ^
