...
export async function create(
  ...
  if (projectDirIsCurrentDir) {
    logger.log(
      `${picocolors.bold(
        nrzGradient(">>> Success!")
      )} Your new Nrz is ready.`
    );
  } else {
    logger.log(
      `${picocolors.bold(
        nrzGradient(">>> Success!")
      )} Created your Nrz at ${picocolors.green(relativeProjectDir)}`
    );
  }
  ...
  if (packageManagerMeta && hasPackageJson) {
    ...
    logger.log("   - Learn more: https://nrz.build/repo/remote-cache");
    logger.log();
    logger.log("- Run commands with Nrz:");
    availableScripts
