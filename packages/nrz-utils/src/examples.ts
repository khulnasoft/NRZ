...
export async function getRepoInfo(
  ...
  if (
    // Support repos whose entire purpose is to be a Nrz example, e.g.
    // https://github.com/:username/:my-cool-nrz-example-repo-name.
    tree === undefined ||
...
export function existsInRepo(nameOrUrl: string): Promise<boolean> {
  try {
    ...
    return isUrlOk(
      `https://api.github.com/repos/khulnasoft/nrz/contents/examples/${encodeURIComponent(
        nameOrUrl
      )}`
    );
  }
...
export async function downloadAndExtractExample(root: string, name: string) {
  const tempFile = await downloadTar(
    `https://codeload.github.com/khulnasoft/nrz/tar.gz/main`,
    `nrz-ct-example`
  );
