...
const logs = window.createOutputChannel("Nrz Extension");
...
export function activate(context: ExtensionContext) {
  ...
  const lspPath = Uri.joinPath(
    ...
    "out",
    `nrz-lsp-${process.platform}-${process.arch}${
  ...
  if (answer === "Install Now") {
    ...
    if (exitCode === 0) {
      ...
      if (message === "Open Docs") {
        env.openExternal(Uri.parse("https://nrz.build/repo/docs/installing"));
      }
    }
