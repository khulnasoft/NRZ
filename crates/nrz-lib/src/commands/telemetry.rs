use nrz_telemetry::{config::TelemetryConfig, events::command::CommandEventBuilder};
use nrz_ui::{color, BOLD, BOLD_GREEN, BOLD_RED};
...
fn log_status(config: TelemetryConfig, base: &CommandBase) {
    ...
    match status {
        ...
        false => {
            ...
            println!(
                "\nYou have opted-out of Nrz anonymous telemetry. No data will be collected from \
                 your machine."
            );
        }
    }
    println!("Learn more: https://nrz.build/repo/docs/telemetry");
}
