...
use nrz_telemetry::events::command::CommandEventBuilder;
use nrz_ui::BOLD;
...
pub async fn prune(
    ...
    if matches!(
        ...
        nrz_repository::package_manager::PackageManager::Bun
    ) {
...
impl<'a> Prune<'a> {
    ...
    fn copy_file(
            ...
            )
        {
            ...
            nrz_fs::copy_file(&from_path, docker_to)?;
        }
        Ok(())
    }
    ...
    fn copy_directory(
        ...
        let full_to = self.full_directory.resolve(path);
        nrz_fs::recursive_copy(&from_path, full_to, self.use_gitignore)?;
        if matches!(destination, Some(CopyDestination::All)) {
            let out_to = self.out_directory.resolve(path);
            nrz_fs::recursive_copy(&from_path, out_to, self.use_gitignore)?;
        }
        if self.docker
            && matches!(
                destination,
        ...
        {
            let docker_to = self.docker_directory().resolve(path);
            nrz_fs::recursive_copy(&from_path, docker_to, self.use_gitignore)?;
        }
        Ok(())
    }
    ...
    fn copy_workspace(&self, package_json_path: &AnchoredSystemPath) -> Result<(), Error> {
        ...
        nrz_fs::recursive_copy(original_dir, &target_dir, self.use_gitignore)?;
