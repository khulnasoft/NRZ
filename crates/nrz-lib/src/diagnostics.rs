                ...
                Ok([version, fsmonitor, untrackedcache]) => {
                    ...
                    if fsmonitor.trim() != "true" || untrackedcache.trim() != "true" {
                        chan.log_line("Git FS Monitor not configured".to_string())
                            .await;
                        chan.log_line( "For more information, see https://nrz.build/repo/docs/reference/command-line-reference/scan#fs-monitor".to_string()).await;
                        let Some(resp) = chan
                            .request(
...
impl Diagnostic for LSPDiagnostic {
    ...
    fn execute(&self, chan: DiagnosticChannel) {
        ...
        tokio::task::spawn(async move {
            chan.started("Nrz Extension".to_string()).await;
            chan.log_line("Checking if extension is running...".to_string())
                .await;
            let pidlock = pidlock::Pidlock::new(lsp_root);
            match pidlock.get_owner() {
                Ok(Some(pid)) => {
                    chan.done(format!("Nrz Extension is running (pid {})", pid))
                        .await;
                }
                Ok(None) => {
                    chan.log_line("Unable to find LSP instance".to_string())
                        .await;
                    chan.log_line( "For more information, see https://nrz.build/repo/docs/reference/command-line-reference/scan#lsp".to_string()).await;
                    chan.failed("Nrz Extension is not running".to_string())
                        .await;
                }
                Err(e) => {
...
impl Diagnostic for UpdateDiagnostic {
    ...
    fn execute(&self, chan: DiagnosticChannel) {
        ...
        tokio::task::spawn(async move {
            chan.started("Update Nrz to latest version".to_string())
                .await;
            ...
            match version {
                Ok(Ok(Some(version))) => {
                    chan.log_line(format!("Nrz {} is available", version).to_string())
                        .await;
                    ...
                    match resp.await.as_ref().map(|s| s.as_str()) {
                        Ok("Yes") => {
                            chan.log_line("Updating Nrz...".to_string()).await;
                            ...
                            match command.wait_with_output().await {
                                Ok(output) if output.status.success() => {
                                    chan.done("Nrz on latest version".to_string()).await;
                                }
                                Ok(output) => {
                                    chan.log_line(
                                    ...
                                    .await;
                                    chan.failed("Unable to update Nrz".to_string()).await
                                }
                                Err(_) => chan.failed("Unable to update Nrz".to_string()).await,
                            }
                        }
                        Ok("No") => chan.failed("Nrz on old version".to_string()).await,
