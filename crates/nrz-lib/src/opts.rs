...
use nrz_api_client::APIAuth;
use nrz_cache::{CacheOpts, RemoteCacheOpts};
...
/// The fully resolved options for Nrz. This is the combination of config,
...
impl<'a> TryFrom<OptsInputs<'a>> for RunOpts {
    ...
    fn try_from(inputs: OptsInputs) -> Result<Self, Self::Error> {
        ...
        let (is_github_actions, log_order, log_prefix) = match inputs.config.log_order() {
            LogOrder::Auto if nrz_ci::Vendor::get_constant() == Some("GITHUB_ACTIONS") => (
                true,
...
impl<'a> TryFrom<OptsInputs<'a>> for CacheOpts {
    ...
    fn try_from(inputs: OptsInputs<'a>) -> Result<Self, Self::Error> {
        let is_linked = nrz_api_client::is_linked(inputs.api_auth);
...
mod test {
    ...
    use nrz_cache::{CacheActions, CacheConfig, CacheOpts};
