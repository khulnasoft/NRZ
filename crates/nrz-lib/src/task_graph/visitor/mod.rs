...
use nrz_ci::{Vendor, VendorBehavior};
use nrz_env::{platform::PlatformEnv, EnvironmentVariableMap};
use nrz_errors::NRZ_SITE;
use nrz_repository::package_graph::{PackageGraph, PackageName, ROOT_PKG_NAME};
use nrz_telemetry::events::{
    generic::GenericEventBuilder, task::PackageTaskEventBuilder, EventBuilder, TrackedErrors,
};
use nrz_ui::{sender::UISender, ColorConfig, ColorSelector, OutputClient, OutputSink, PrefixedUI};
...
pub enum Error {
    ...
    #[error(
        "'{package}' is configured with a {mfe_config_filename}, but doesn't have \
         '@khulnasoft/microfrontends' listed as a dependency."
    )]
            ...
            match self.dry {
                ...
                false => {
                    ...
                    let Some(mut exec_context) = factory.exec_context(
                        ...
                        // TODO(gsoltis): if/when we fix https://github.com/khulnasoft/nrz/issues/937
    ...
    fn output_client(
        ...
        let behavior = match self.run_opts.log_order {
            crate::opts::ResolvedLogOrder::Stream if self.run_tracker.spaces_enabled() => {
                nrz_ui::OutputClientBehavior::InMemoryBuffer
            }
            crate::opts::ResolvedLogOrder::Stream => nrz_ui::OutputClientBehavior::Passthrough,
            crate::opts::ResolvedLogOrder::Grouped => nrz_ui::OutputClientBehavior::Grouped,
        };
