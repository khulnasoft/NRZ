...
use either::Either;
use nrz_ui::{sender::TaskSender, tui::event::CacheResult, OutputClient, OutputWriter, PrefixedUI};
...
impl<W: Write> TaskCacheOutput<W> {
    pub fn task_writer(&mut self) -> Either<nrz_ui::PrefixedWriter<&mut W>, TaskSender> {
        match self {
...
impl<W: Write> CacheOutput for TaskCacheOutput<W> {
    ...
    fn replay_logs(&mut self, log_file: &AbsoluteSystemPath) -> Result<(), nrz_ui::Error> {
        match self {
            TaskCacheOutput::Direct(direct) => {
                let writer = direct.output_prefixed_writer();
                nrz_ui::replay_logs(writer, log_file)
            }
            TaskCacheOutput::UI(task) => nrz_ui::replay_logs(task, log_file),
        }
    }
}
