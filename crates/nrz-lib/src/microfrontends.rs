...
use nrz_microfrontends::{Config as MFEConfig, Error, MICROFRONTENDS_PACKAGE};
    ...
    fn new<'a>(
        ...
        for (package_name, config) in sorted_packages {
            ...
            let Some(config) = config.or_else(|err| match err {
                nrz_microfrontends::Error::UnsupportedVersion(_) => {
                    unsupported_version.push((package_name.to_string(), err.to_string()));
                    Ok(None)
                }
                nrz_microfrontends::Error::ChildConfig { reference } => {
...
#[cfg(test)]
mod test {
    use nrz_microfrontends::MICROFRONTENDS_PACKAGE;
    use serde_json::json;
