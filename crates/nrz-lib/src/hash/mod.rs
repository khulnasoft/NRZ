//! hash module
//!
//! This module contains the hash functions used by nrz for certain
...
use nrz_env::EnvironmentVariablePairs;
...
pub struct LockFilePackages(pub Vec<nrz_lockfiles::Package>);
...
impl From<LockFilePackages> for Builder<HeapAllocator> {
    fn from(LockFilePackages(packages): LockFilePackages) -> Self {
...

        {
            ...
            for (i, nrz_lockfiles::Package { key, version }) in packages.iter().enumerate() {
