...
use nrz_api_client::{spaces::CreateSpaceRunPayload, APIAuth, APIClient};
use nrz_env::EnvironmentVariableMap;
use nrz_repository::package_graph::{PackageGraph, PackageName};
use nrz_scm::SCM;
use nrz_ui::{color, cprintln, cwriteln, ColorConfig, BOLD, BOLD_CYAN, GREY};
...
pub enum Error {
    ...
    SpacesRequest(#[from] nrz_api_client::Error),
    ...
    async fn send_to_space(
        ...
        let spinner = (!is_watch).then(|| {
            tokio::spawn(async {
                ...
                nrz_ui::start_spinner("...sending run summary...");
            })
        });
