...
use nrz_env::{DetailedMap, EnvironmentVariableMap};
...
impl From<nrz_cache::CacheSource> for CacheSource {
    fn from(value: nrz_cache::CacheSource) -> Self {
        match value {
            nrz_cache::CacheSource::Local => Self::Local,
            nrz_cache::CacheSource::Remote => Self::Remote,
        }
    }
}
...
impl TaskEnvVarSummary {
    pub fn new(
        ...
        let pass_through = task_definition
            .pass_through_env
            .as_deref()
            .map(|pass_through_env| -> Result<_, nrz_env::Error> {
