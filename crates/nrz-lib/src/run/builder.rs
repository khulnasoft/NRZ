...
use chrono::Local;
use nrz_analytics::{start_analytics, AnalyticsHandle, AnalyticsSender};
use nrz_api_client::{APIAuth, APIClient};
use nrz_cache::AsyncCache;
use nrz_env::EnvironmentVariableMap;
...
use nrz_ui::{ColorConfig, ColorSelector};
...
impl RunBuilder {
    ...
    pub async fn build(
        ...
        let is_linked = nrz_api_client::is_linked(&self.api_auth);
        ...
        let is_ci_or_not_tty = nrz_ci::is_ci() || !std::io::stdout().is_terminal();
        run_telemetry.track_ci(nrz_ci::Vendor::get_name());
        ...
        let mut pkg_dep_graph = {
                ...
                && !self.opts.repo_opts.allow_no_package_manager
            {
                match (&daemon, self.opts.run_opts.daemon) {
                    (None, Some(true)) => {
                        ...
                        return Err(nrz_repository::package_graph::Error::Discovery(
            ...
            match graph {
                ...
                Err(nrz_repository::package_graph::Error::PackageJson(
                    package_json::Error::Io(io),
                )) if io.kind() == ErrorKind::NotFound => {
                    run_telemetry.track_error(TrackedErrors::InvalidPackageDiscovery);
                    return Err(nrz_repository::package_graph::Error::PackageJson(
                        package_json::Error::Io(io),
                    )
                    .into());
                }
