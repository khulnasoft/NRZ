...
use nrz_cache::CacheConfig;
use nrz_errors::NRZ_SITE;
use nrz_json::NrzJsonReader;
use nrz_repository::package_graph::PackageName;
...
pub enum Error {
    ...
    #[error(
        "Could not find nrz.json.\nFollow directions at https://nrz.build/repo/docs to create one."
    )]
    NoNrzJSON,
    #[error(transparent)]
    ...
    #[error(transparent)]
    Cache(#[from] nrz_cache::config::Error),
    #[error(
    ...
    ApiClient(#[source] nrz_api_client::Error),
    #[error("{0} is not UTF8.")]
    Encoding(String),
...
const DEFAULT_API_URL: &str = "https://khulnasoft.com/api";
const DEFAULT_LOGIN_URL: &str = "https://khulnasoft.com";
...
#[setters(
    prefix = "with_",
    generate_delegates(ty = "NrzConfigBuilder", field = "override_config")
)]
...
mod test {
    ...
    use crate::config::{
        ConfigurationOptions, NrzConfigBuilder, DEFAULT_API_URL, DEFAULT_LOGIN_URL, DEFAULT_TIMEOUT,
    };
    ...
    fn test_env_layering() {
        ...
        let khulnasoft_artifacts_owner = "team_SOMEHASH";
