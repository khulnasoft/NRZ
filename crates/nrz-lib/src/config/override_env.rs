...
1. The general pattern is that:
  - ConfigurationOptions.token corresponds to NRZ_TOKEN or KHULNASOFT_ARTIFACTS_TOKEN
  - ConfigurationOptions.team_id corresponds to NRZ_TEAMID or KHULNASOFT_ARTIFACTS_OWNER
...
1. We're ultimately poking around the env vars looking for _pairs_ that make sense.
   Since we presume that users are the only ones sending NRZ_* and Khulnasoft is the only one sending KHULNASOFT_*, we can make some assumptions.  Namely, we assume that if we have one of KHULNASOFT_ARTIFACTS_OWNER or KHULNASOFT_ARTIFACTS_TOKEN we will always have both.
...
struct Input {
    ...
    KHULNASOFT_ARTIFACTS_OWNER: Option<String>,
    KHULNASOFT_ARTIFACTS_TOKEN: Option<String>,
}
...
impl<'a> TryFrom<&'a HashMap<OsString, OsString>> for Input {
    ...
    fn try_from(environment: &'a HashMap<OsString, OsString>) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            KHULNASOFT_ARTIFACTS_OWNER: get_value("khulnasoft_artifacts_owner")?,
            KHULNASOFT_ARTIFACTS_TOKEN: get_value("khulnasoft_artifacts_token")?,
        })
    }
}
...
impl From<Input> for Output {
    fn from(input: Input) -> Self {
        ...
        // if there's both Khulnasoft items, we use those next
            ...
            Output {
                ...
                team_slug: input.NRZ_TEAM, /* this may or may not be Some, but if it is we can
                                            * pass it along too */
                token: input.KHULNASOFT_ARTIFACTS_TOKEN,
            }
        }
        ...
        // ------------------------------------------------
            ...
            Output {
                // prefer NRZ_TEAMID to KHULNASOFT_ARTIFACTS_OWNER
                team_id: input.NRZ_TEAMID.or(input.KHULNASOFT_ARTIFACTS_OWNER),
...
mod test {
    ...
    const KHULNASOFT_ARTIFACTS_OWNER: &str = "valueof:KHULNASOFT_ARTIFACTS_OWNER";
    const KHULNASOFT_ARTIFACTS_TOKEN: &str = "valueof:KHULNASOFT_ARTIFACTS_TOKEN";
    ...
    impl TestCase {
        ...
        #[allow(non_snake_case)]
        fn KHULNASOFT_ARTIFACTS_OWNER(mut self) -> Self {
            self.input.KHULNASOFT_ARTIFACTS_OWNER = Some(KHULNASOFT_ARTIFACTS_OWNER.into());
            self
        }

        #[allow(non_snake_case)]
        fn KHULNASOFT_ARTIFACTS_TOKEN(mut self) -> Self {
            self.input.KHULNASOFT_ARTIFACTS_TOKEN = Some(KHULNASOFT_ARTIFACTS_TOKEN.into());
            self
        }
    ...
    fn test_all_the_combos() {
        let cases: &[TestCase] = &[
            ...
            TestCase::new()
                .reason("just KHULNASOFT_ARTIFACTS_TOKEN")
                .KHULNASOFT_ARTIFACTS_TOKEN(),
            ...
            // ------------------------------
            TestCase::new()
                .reason("golden path for 3rd party, not deployed on Khulnasoft")
                .NRZ_TEAM()
                .NRZ_TOKEN()
                .team_slug(NRZ_TEAM)
                .token(NRZ_TOKEN),
            TestCase::new()
                ...
                .NRZ_TEAM()
                .NRZ_TOKEN()
                .KHULNASOFT_ARTIFACTS_TOKEN() // disregarded
                .team_slug(NRZ_TEAM)
                .token(NRZ_TOKEN),
            TestCase::new()
                .reason("golden path for 3rd party, deployed on Khulnasoft")
                .NRZ_TEAM()
                .NRZ_TOKEN()
                .KHULNASOFT_ARTIFACTS_OWNER() // normally this would map to team_id, but not with a complete 3rd party pair
                .KHULNASOFT_ARTIFACTS_TOKEN()
                .team_slug(NRZ_TEAM)
                .token(NRZ_TOKEN),
            //
            ...
            TestCase::new()
                ...
                .token(NRZ_TOKEN),
            TestCase::new()
                .reason("a NRZ_TEAMID+NRZ_TOKEN pair will also win against a Khulnasoft pair")
                .NRZ_TEAMID()
                .NRZ_TOKEN()
                .KHULNASOFT_ARTIFACTS_OWNER()
                .KHULNASOFT_ARTIFACTS_TOKEN()
            ...
            TestCase::new()
                .reason(
                    "a NRZ_TEAMID+NRZ_TOKEN pair wins against an incomplete Khulnasoft (just \
                     artifacts token)",
                )
                .NRZ_TEAMID()
                .NRZ_TOKEN()
                ...
                .token(NRZ_TOKEN),
            //
            // When Khulnasoft Wins
            // ------------------------------
            TestCase::new()
                .reason("golden path on Khulnasoft zero config")
                .KHULNASOFT_ARTIFACTS_OWNER()
                .KHULNASOFT_ARTIFACTS_TOKEN()
                .team_id(KHULNASOFT_ARTIFACTS_OWNER)
                .token(KHULNASOFT_ARTIFACTS_TOKEN),
            TestCase::new()
                .reason("Khulnasoft wins: disregard just NRZ_TOKEN")
                .NRZ_TOKEN()
                .KHULNASOFT_ARTIFACTS_OWNER()
                .KHULNASOFT_ARTIFACTS_TOKEN()
                .team_id(KHULNASOFT_ARTIFACTS_OWNER)
                .token(KHULNASOFT_ARTIFACTS_TOKEN),
            TestCase::new()
                .reason("Khulnasoft wins: NRZ_TEAM can join in the fun if it wants")
                .NRZ_TEAM()
                .KHULNASOFT_ARTIFACTS_OWNER()
                .KHULNASOFT_ARTIFACTS_TOKEN()
                .team_id(KHULNASOFT_ARTIFACTS_OWNER)
                ...
                .token(KHULNASOFT_ARTIFACTS_TOKEN),
            TestCase::new()
                .reason("Khulnasoft wins: disregard just NRZ_TEAMID")
                .NRZ_TEAMID()
                .KHULNASOFT_ARTIFACTS_OWNER()
                .KHULNASOFT_ARTIFACTS_TOKEN()
                .team_id(KHULNASOFT_ARTIFACTS_OWNER)
                .token(KHULNASOFT_ARTIFACTS_TOKEN),
            TestCase::new()
                .reason("Khulnasoft wins if NRZ_TOKEN is missing")
                .NRZ_TEAM()
                .NRZ_TEAMID()
                .KHULNASOFT_ARTIFACTS_OWNER()
                .KHULNASOFT_ARTIFACTS_TOKEN()
            ...
            // ------------------------------
            TestCase::new()
                .reason("just KHULNASOFT_ARTIFACTS_OWNER")
                .KHULNASOFT_ARTIFACTS_OWNER()
                .team_id(KHULNASOFT_ARTIFACTS_OWNER),
            TestCase::new()
        ...
        for case in cases {
            ...
            if let Some(value) = &case.input.KHULNASOFT_ARTIFACTS_OWNER {
                env.insert("khulnasoft_artifacts_owner".into(), value.into());
            }
            if let Some(value) = &case.input.KHULNASOFT_ARTIFACTS_TOKEN {
                env.insert("khulnasoft_artifacts_token".into(), value.into());
            }
