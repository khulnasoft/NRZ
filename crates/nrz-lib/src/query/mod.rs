...
use nrz_repository::{change_mapper::AllPackageChangeReason, package_graph::PackageName};
...
impl RepositoryQuery {
    ...
    fn convert_change_reason(
        &self,
        reason: nrz_repository::change_mapper::PackageInclusionReason,
    ) -> PackageChangeReason {
        match reason {
            nrz_repository::change_mapper::PackageInclusionReason::All(
            ...
            nrz_repository::change_mapper::PackageInclusionReason::RootTask { task } => {
                PackageChangeReason::RootTask(RootTask {
                    task_name: task.to_string(),
                })
            }
            nrz_repository::change_mapper::PackageInclusionReason::ConservativeRootLockfileChanged => {
                PackageChangeReason::ConservativeRootLockfileChanged(ConservativeRootLockfileChanged { empty: false })
            }
            nrz_repository::change_mapper::PackageInclusionReason::LockfileChanged { removed, added } => {
            ...
            nrz_repository::change_mapper::PackageInclusionReason::DependencyChanged {
                dependency,
            } => PackageChangeReason::DependencyChanged(DependencyChanged {
                dependency_name: dependency.to_string(),
            }),
            nrz_repository::change_mapper::PackageInclusionReason::DependentChanged {
                dependent,
            } => PackageChangeReason::DependentChanged(DependentChanged {
                dependent_name: dependent.to_string(),
            }),
            nrz_repository::change_mapper::PackageInclusionReason::FileChanged { file } => {
                PackageChangeReason::FileChanged(FileChanged {
                    file_path: file.to_string(),
                })
            }
            nrz_repository::change_mapper::PackageInclusionReason::InFilteredDirectory {
                directory,
            } => PackageChangeReason::InFilteredDirectory(InFilteredDirectory {
                directory_path: directory.to_string(),
            }),
            nrz_repository::change_mapper::PackageInclusionReason::IncludedByFilter {
                filters,
