...
use nrz_graph_utils as graph;
use nrz_lockfiles::Lockfile;
use nrzpath::{
...
///
/// It follows the Khulnasoft glossary of terms where "Workspace"
/// is the collection of packages and "Package" is a single package within the
/// workspace. https://khulnasoft.com/docs/khulnasoft-platform/glossary
...
pub struct PackageInfo {
    ...
    pub transitive_dependencies: Option<HashSet<nrz_lockfiles::Package>>,
}
...
pub struct ExternalDependencyChange {
    ...
    pub added: Vec<nrz_lockfiles::Package>,
    /// Dependencies that were removed from the package
    pub removed: Vec<nrz_lockfiles::Package>,
}
...
impl PackageGraph {
    ...
    pub fn dependencies<'a>(&'a self, node: &PackageNode) -> HashSet<&'a PackageNode> {
        let mut dependencies = nrz_graph_utils::transitive_closure(
            &self.graph,
    ...
    pub fn ancestors(&self, node: &PackageNode) -> HashSet<&PackageNode> {
        ...
        let mut dependents = if self.root_internal_dependencies().contains(node) {
            ...
            nrz_graph_utils::transitive_closure(
                &self.graph,
                self.node_lookup.get(node).cloned(),
    ...
    pub fn changed_packages_from_lockfile(
        ...
        let closures = nrz_lockfiles::all_transitive_closures(previous, external_deps, true)?;
...
pub enum ChangedPackagesError {
    ...
    Lockfile(#[from] nrz_lockfiles::Error),
}
...
mod test {
    ...
    #[derive(Debug)]
    struct MockLockfile {}
    impl nrz_lockfiles::Lockfile for MockLockfile {
        fn resolve_package(
            &self,
            _workspace_path: &str,
            name: &str,
            _version: &str,
        ) -> std::result::Result<Option<nrz_lockfiles::Package>, nrz_lockfiles::Error> {
            Ok(match name {
                "a" => Some(nrz_lockfiles::Package::new("key:a", "1")),
                "b" => Some(nrz_lockfiles::Package::new("key:b", "1")),
                "c" => Some(nrz_lockfiles::Package::new("key:c", "1")),
                _ => None,
            })
        }
        ...
        fn all_dependencies(
            &self,
            key: &str,
        ) -> std::result::Result<Option<HashMap<String, String>>, nrz_lockfiles::Error> {
            match key {
                "key:a" => Ok(Some(
                    [("c", "1")]
                        .iter()
        ...
        fn subgraph(
            ...
            _packages: &[String],
        ) -> std::result::Result<Box<dyn Lockfile>, nrz_lockfiles::Error> {
            unreachable!("lockfile pruning not necessary for package graph construction")
        }

        fn encode(&self) -> std::result::Result<Vec<u8>, nrz_lockfiles::Error> {
    ...
    async fn test_lockfile_traversal() {
        ...
        let bar_deps = pkg_graph
            ...
            .as_ref()
            .unwrap();
        let a = nrz_lockfiles::Package::new("key:a", "1");
        let b = nrz_lockfiles::Package::new("key:b", "1");
        let c = nrz_lockfiles::Package::new("key:c", "1");
