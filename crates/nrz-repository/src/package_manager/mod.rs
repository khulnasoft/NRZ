    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err = match self.package_manager {
            PackageManager::Pnpm | PackageManager::Pnpm6 | PackageManager::Pnpm9 => {
                "pnpm-workspace.yaml: no packages found. Nrz requires pnpm workspaces and thus \
                 packages to be defined in the root pnpm-workspace.yaml"
            }
            PackageManager::Yarn | PackageManager::Berry => {
                "package.json: no workspaces found. Nrz requires yarn workspaces to be defined in \
                 the root package.json"
            }
            PackageManager::Npm => {
                "package.json: no workspaces found. Nrz requires npm workspaces to be defined in \
                 the root package.json"
            }
            PackageManager::Bun => {
                "package.json: no workspaces found. Nrz requires bun workspaces to be defined in \
                 the root package.json"
            }
        };
        write!(f, "{}", err)
...
impl PackageManager {
    ...
    pub fn parse_lockfile(
        ...
        Ok(match self {
            PackageManager::Npm => Box::new(nrz_lockfiles::NpmLockfile::load(contents)?),
            PackageManager::Pnpm | PackageManager::Pnpm6 | PackageManager::Pnpm9 => {
                Box::new(nrz_lockfiles::PnpmLockfile::from_bytes(contents)?)
            }
            PackageManager::Yarn => Box::new(nrz_lockfiles::Yarn1Lockfile::from_bytes(contents)?),
            PackageManager::Bun => Box::new(nrz_lockfiles::BunLockfile::from_bytes(contents)?),
            PackageManager::Berry => Box::new(nrz_lockfiles::BerryLockfile::load(
                contents,
                Some(nrz_lockfiles::BerryManifest::with_resolutions(
                    root_package_json
                        .resolutions
                        .iter()
                        .flatten()
...
mod tests {
    ...
    fn repo_root() -> AbsoluteSystemPathBuf {
        ...
        panic!("Couldn't find Nrz root from {}", cwd);
    }
