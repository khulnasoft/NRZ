...
//! Nrz's analytics library. Handles sending analytics events to the
//! Khulnasoft API in the background. We only record cache usage events,
//! so when the cache is hit or missed for the file system or the HTTP cache.
//! Requires the user to be logged in to Khulnasoft.

use std::time::Duration;

use futures::{stream::FuturesUnordered, StreamExt};
use nrz_api_client::{analytics::AnalyticsClient, APIAuth};
pub use nrz_khulnasoft_api::AnalyticsEvent;
use thiserror::Error;
use tokio::{
    select,
    sync::{mpsc, oneshot},
...
mod tests {
    ...
    use nrz_api_client::{analytics::AnalyticsClient, APIAuth};
    use nrz_khulnasoft_api::{AnalyticsEvent, CacheEvent, CacheSource};
    use tokio::{
        select,
