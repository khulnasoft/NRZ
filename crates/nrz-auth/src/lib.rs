...
#![deny(clippy::all)]
//! Nrz's library for authenticating with the Khulnasoft API.
//! Handles logging into Khulnasoft, verifying SSO, and storing the token.
...
use nrz_khulnasoft_api::{token::ResponseTokenMetadata, User};
...
pub const KHULNASOFT_TOKEN_DIR: &str = "com.khulnasoft.cli";
pub const KHULNASOFT_TOKEN_FILE: &str = "auth.json";
pub const NRZ_TOKEN_DIR: &str = "nrz";
    ...
    pub async fn has_cache_access<T: CacheClient>(
            ...
            .await
        {
            ...
            Err(e) => match e {
                ...
                nrz_api_client::Error::ReqwestError(e)
...
mod tests {
    ...
    use nrz_khulnasoft_api::{token::Scope, CachingStatus, CachingStatusResponse};
        ...
        async fn get_caching_status(
            ...
            match self.response {
                ...
                MockCachingResponse::Error(MockErrorType::Error) => {
                    Err(nrz_api_client::Error::UnknownStatus {
