...
pub use error::Error;
use nrz_api_client::{CacheClient, Client, TokenClient};
use nrz_ui::{start_spinner, ColorConfig, BOLD};
use reqwest::Url;
use tokio::sync::OnceCell;
use tracing::{debug, warn};

use crate::{auth::extract_khulnasoft_token, error, ui, LoginOptions, Token};
...
/// to Khulnasoft, checks if the user has a Khulnasoft CLI token on disk.
pub async fn login<T: Client + TokenClient + CacheClient>(
    ...
    if !force {
        ...
        // If the user is logging into Khulnasoft, check for an existing `vc` token.
            ...
            if let Ok(Some(token)) = extract_khulnasoft_token() {
                debug!("found existing Khulnasoft token");
                let token = Token::existing(token);
                if token
                    .is_valid(
                        api_client,
                        Some(valid_token_callback(
                            "Existing Khulnasoft token found!",
                            color_config,
                        )),
                    )
                    .await?
...
mod tests {
    ...
    impl LoginServer for MockLoginServer {
        async fn run(
            ...
            login_token
                .set(nrz_khulnasoft_api_mock::EXPECTED_TOKEN.to_string())
                .unwrap();
            Ok(())
        }
    }
    ...
    impl From<MockApiError> for nrz_api_client::Error {
        fn from(error: MockApiError) -> Self {
            match error {
                MockApiError::EmptyToken => nrz_api_client::Error::UnknownStatus {
    ...
    impl Client for MockApiClient {
        async fn get_user(&self, token: &str) -> nrz_api_client::Result<UserResponse> {
            if token.is_empty() {
        ...
        async fn get_teams(&self, token: &str) -> nrz_api_client::Result<TeamsResponse> {
            if token.is_empty() {
                return Err(MockApiError::EmptyToken.into());
            }
        ...
        async fn get_team(
            ...
            _token: &str,
            _team_id: &str,
        ) -> nrz_api_client::Result<Option<Team>> {
        ...
        async fn get_spaces(
            ...
            _token: &str,
            _team_id: Option<&str>,
        ) -> nrz_api_client::Result<SpacesResponse> {
        ...
        async fn verify_sso_token(
            ...
            token: &str,
            _: &str,
        ) -> nrz_api_client::Result<VerifiedSsoUser> {
            Ok(VerifiedSsoUser {
                token: token.to_string(),
                team_id: Some("team_id".to_string()),
            })
        }
        async fn handle_403(_response: Response) -> nrz_api_client::Error {
            unimplemented!("handle_403")
        }
        fn make_url(&self, endpoint: &str) -> nrz_api_client::Result<Url> {
            let url = format!("{}{}", self.base_url, endpoint);
            Url::parse(&url).map_err(|err| nrz_api_client::Error::InvalidUrl { url, err })
        }
    }
    ...
    impl TokenClient for MockApiClient {
        async fn get_metadata(
            &self,
            token: &str,
        ) -> nrz_api_client::Result<nrz_khulnasoft_api::token::ResponseTokenMetadata> {
            if token.is_empty() {
                return Err(MockApiError::EmptyToken.into());
            }

            Ok(nrz_khulnasoft_api::token::ResponseTokenMetadata {
                id: "id".to_string(),
                name: "name".to_string(),
                token_type: "token".to_string(),
                origin: "github".to_string(),
                scopes: vec![nrz_khulnasoft_api::token::Scope {
        ...
        async fn delete_token(&self, _token: &str) -> nrz_api_client::Result<()> {
            Ok(())
        }
    }
    ...
    impl CacheClient for MockApiClient {
        async fn get_artifact(
            ...
            _method: Method,
        ) -> Result<Option<Response>, nrz_api_client::Error> {
            unimplemented!("get_artifact")
        }
        async fn put_artifact(
            &self,
            _hash: &str,
            _artifact_body: impl nrz_api_client::Stream<Item = Result<nrz_api_client::Bytes, nrz_api_client::Error>>
                + Send
                + Sync
                + 'static,
            _body_len: usize,
        ...
        async fn get_caching_status(
            ...
            _team_id: Option<&str>,
            _team_slug: Option<&str>,
        ) -> Result<CachingStatusResponse, nrz_api_client::Error> {
    ...
    async fn test_login() {
        ...
        assert_eq!(&got_token, nrz_khulnasoft_api_mock::EXPECTED_TOKEN);
