...
use nrz_analytics::AnalyticsSender;
use nrz_api_client::{
...
impl HTTPCache {
    ...
    pub async fn put(
        ...
        .map(|res| {
            res.map(|bytes| bytes.freeze())
                .map_err(nrz_api_client::Error::from)
        });
        ...
        let body = if let Some(signer_verifier) = &self.signer_verifier {
            ...
            let body = response.bytes().await.map_err(|e| {
                CacheError::ApiClientError(
                    Box::new(nrz_api_client::Error::ReqwestError(e)),
                    Backtrace::capture(),
                )
            })?;
    ...
    fn convert_api_error(hash: &str, err: nrz_api_client::Error) -> CacheError {
        match err {
            nrz_api_client::Error::ReqwestError(e) if e.is_timeout() => {
                CacheError::TimeoutError(hash.to_string())
            }
            nrz_api_client::Error::ReqwestError(e) if e.is_connect() => CacheError::ConnectError,
            nrz_api_client::Error::UnknownStatus { code, .. } if code == "forbidden" => {
...
mod test {
    ...
    use nrz_analytics::start_analytics;
    use nrz_api_client::{analytics, APIClient};
    ...
    fn test_forbidden_error() {
        let err = HTTPCache::convert_api_error(
            "hash",
            nrz_api_client::Error::UnknownStatus {
                code: "forbidden".into(),
                message: "Not authorized".into(),
                backtrace: Backtrace::capture(),
            },
        );
        assert_snapshot!(err.to_string(), @"Insufficient permissions to write to remote cache. Please verify that your role has write access for Remote Cache Artifact at https://khulnasoft.com/docs/accounts/team-members-and-roles/access-roles/team-level-roles?resource=Remote+Cache+Artifact");
    }
    ...
    fn test_cache_disabled() {
        let err = HTTPCache::convert_api_error(
            "hash",
            nrz_api_client::Error::CacheDisabled {
                status: nrz_khulnasoft_api::CachingStatus::Disabled,
