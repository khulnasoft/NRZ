...
const DEFAULT_ENV_VARS: &[&str] = ["KHULNASOFT_ANALYTICS_ID", "KHULNASOFT_TARGET_ENV"].as_slice();
...
mod tests {
    ...
    #[test_case(&[], &["KHULNASOFT_ANALYTICS_ID", "KHULNASOFT_TARGET_ENV"] ; "defaults")]
    #[test_case(&["!KHULNASOFT*"], &[] ; "removing defaults")]
    #[test_case(&["FOO*", "!FOOD"], &["FOO", "FOOBAR", "KHULNASOFT_ANALYTICS_ID", "KHULNASOFT_TARGET_ENV"] ; "intersecting globs")]
    fn test_global_env(inputs: &[&str], expected: &[&str]) {
        let env_at_start = EnvironmentVariableMap(
            vec![
                ("KHULNASOFT_TARGET_ENV", "prod"),
                ("KHULNASOFT_ANALYTICS_ID", "1"),
                ("FOO", "bar"),
                ("FOOBAR", "baz"),
                ("FOOD", "cheese"),
            ]
