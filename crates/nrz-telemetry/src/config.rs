...
///
/// https://github.com/khulnasoft/nrz/blob/main/packages/nrz-telemetry/src/config.ts
use std::env;

use chrono::{DateTime, Utc};
pub use config::{Config, ConfigError, File, FileFormat};
use nrz_dirs::config_dir;
use nrz_ui::{color, ColorConfig, BOLD, GREY, UNDERLINE};
...
impl TelemetryConfig {
    ...
    pub fn new(config_path: AbsoluteSystemPathBuf) -> Result<TelemetryConfig, ConfigError> {
        ...
        // isolated from other nrz config, try to remove the entire config
...
fn default_config_path() -> Result<AbsoluteSystemPathBuf, ConfigError> {
    ...
    Ok(config_dir.join_components(&["nrz", "telemetry.json"]))
}
