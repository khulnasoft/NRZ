//! Nrz's telemetry library. Handles sending anonymous telemetry events to
//! the Khulnasoft API in the background.
//!
//! More detail is available at https://nrz.build/repo/docs/telemetry.
...
use nrz_api_client::telemetry;
use nrz_ui::{color, ColorConfig, BOLD, GREY};
...
mod tests {
    ...
    async fn test_batching() {
        ...
        let config =
            TelemetryConfig::new(temp_dir.join_components(&["nrz", "telemetry.json"])).unwrap();
