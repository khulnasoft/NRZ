            ...
            let daemon = match daemon {
                ...
                Err(DaemonConnectorError::Handshake(box DaemonError::VersionMismatch(message))) => {
                    self.client
                        .show_message(
                            MessageType::ERROR,
                            "Pre-2.0 versions of nrz are not compatible with 2.0 or later of the \
                             extension. If you do not plan to update to nrz 2.0, please ensure \
                             you install the latest 1.0 version of the extension in this \
                             workspace.",
                        )
                        .await;
                    self.client
