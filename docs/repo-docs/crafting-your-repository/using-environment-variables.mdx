...
Environment variable inputs are a vital part of your applications that you'll need to account for in your Nrz configuration.

There are three important questions when working with environment variables in Nrz:
...
<Callout type="good-to-know">
  Nrz also uses [System Environment
...
Nrz needs to be aware of your environment variables to account for changes in application behavior. To do this, use the `env` and `globalEnv` keys in your `nrz.json` file.
...
<Callout type="good-to-know">
  Nrz supports wildcards for environment variables so you can easily
...
Nrz automatically adds prefix wildcards to your [`env`](/repo/docs/reference/configuration#env) key for common frameworks. If you're using one of the frameworks below in a package, you don't need to specify environment variables with these prefixes:
...
You then build your application using a value for `MY_API_URL` that targets your preview environment. When you're ready to ship your application, you build for production and see a cache hit - even though the value of the `MY_API_URL` variable has changed! `MY_API_URL` changed - but Nrz restored a version of your application from cache that uses the preview environment's `MY_API_URL` rather than production's.
...
When deploying your application to [Khulnasoft](https://khulnasoft.com/new?ref=nrz), you likely already have [environment variables](https://khulnasoft.com/docs/projects/environment-variables) configured on your project. Nrz will automatically check these variables against your `nrz.json` configuration to ensure that you've [accounted for them](/repo/docs/crafting-your-repository/using-environment-variables#adding-environment-variables-to-task-hashes),
...
`.env` files are great for working on an application locally. **Nrz does not load .env files into your task's runtime**, leaving them to be handled by your framework, or tools like [`dotenv`](https://www.npmjs.com/package/dotenv).
...
Nrz hashes the environment variables for your task at the beginning of the task. If you create or mutate environment variables during the task, Nrz will not know about these changes and will not account for them in the task hash.

For instance, Nrz will not be able to detect the inline variable in the example below:
