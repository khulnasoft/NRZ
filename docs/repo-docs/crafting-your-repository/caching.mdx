---
title: Caching
description: Learn about caching in Nrz.
---
...
Nrz uses caching to speed up builds, ensuring you **never do the same work twice**. When your task is cacheable, Nrz will restore the results of your task from cache using a fingerprint from the first time the task ran.

![12 tasks are being ran in 3 packages, resulting in a ">>> FULL NRZ" cache hit. The total time it takes to restore these tasks from cache is 80 milliseconds.](/images/docs/why-nrz-solution.png)

Nrz's caching results in significant time savings when working locally - and is even more powerful when [Remote Caching](/repo/docs/core-concepts/remote-caching) is enabled, sharing a cache among your entire team and CI.

On this page, you'll learn:

- [How to hit your first Nrz cache](#hit-your-first-nrz-cache)
- [How to enable Remote Caching](/repo/docs/core-concepts/remote-caching)
- [What Nrz uses for the inputs and outputs to a hash](/repo/docs/crafting-your-repository/caching#task-inputs)
- [How to troubleshoot caching issues](#troubleshooting)

<Callout type="good-to-know">
  Nrz assumes that your tasks are **deterministic**. If a task is able to
  produce different outputs given the set of inputs that Nrz is aware of,
  caching may not work as expected.
</Callout>

## Hit your first Nrz cache

You can try out Nrz's caching behavior in three steps:

<Steps>
<Step>
### Create a new Nrz project

Use `npx create-nrz@latest` and follow the prompts to create a new Nrz.
...
Nrz stores the results of tasks in the `.nrz/cache` directory on your machine. However, you can make your entire organization even faster by sharing this cache with your teammates and CI.
...
Now, when you run a task, Nrz will automatically send the outputs of the task to Remote Cache. If you run the same task on a different machine that is also authenticated to your Remote Cache, it will hit cache the first time it runs the task.
...
<Callout type="info">
  By default, Nrz uses [Khulnasoft Remote
...
Nrz caches two types of outputs: Task outputs and Logs.

### Task outputs

Nrz caches the file outputs of a task that are defined in [the `outputs` key](/repo/docs/reference/configuration#outputs) of `nrz.json`. When there's a cache hit, Nrz will restore the files from the cache.

The `outputs` key is optional, see [the API reference](/repo/docs/reference/configuration#outputs) for how Nrz behaves in this case.

<Callout type="warn" title="Providing file outputs">
If you do not declare file outputs for a task, Nrz will not cache them. This might be okay for some tasks (like linters) - but many tasks produce files that you will want to be cached.
...
Nrz always captures the terminal outputs of your tasks, restoring those logs to your terminal from the first time that the task ran.
...
Inputs are hashed by Nrz, creating a "fingerprint" for the task run. When "fingerprints" match, running the task will hit the cache.

Under the hood, Nrz creates two hashes: a global hash and a task hash. If either of the hashes change, the task will miss cache.
...
Nrz has a [`--dry` flag](/repo/docs/reference/run#--dry----dry-run) that can be used to see what would happen if you ran a task without actually running it. This can be useful for debugging caching issues when you're not sure which tasks you're running.

For more details, visit the [`--dry` API reference](/repo/docs/reference/run#--dry----dry-run).

### Using Run Summaries

Nrz has a [`--summarize` flag](/repo/docs/reference/run#--summarize) that can be used to get an overview of all of a task's inputs, outputs, and more. Comparing two summaries will show why two task's hashes are different. This can be useful for:

- Debugging inputs: There are many inputs to a task in Nrz. If a task is missing cache when you expect it to hit, you can use a Run Summary to check which inputs are different that you weren't expecting.
...
<Callout type="info" title="Summaries viewer">
  While there is not a Nrz-native Run Summaries UI viewer, we encourage
...
- **Scripts that have their own caching**: Some tasks have their own internal caching behavior. In these cases, configuration can quickly become complicated to make Nrz's cache and the application cache work together.
...
Now that you've seen how Nrz's caching makes your repository faster, let's take a look at how to develop applications and libraries in your Nrz.
