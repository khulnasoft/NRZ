...
Most development tasks are long-running tasks that watch for changes to your code. Nrz enhances this experience with a powerful terminal UI and other capabilities like:
...
Defining a development task in `nrz.json` tells Nrz that you'll be running a long-lived task. This is useful for things like running a development server, running tests, or building your application.
...
- **"cache": false**: Tells Nrz to not attempt to cache the results of the task. Since this is a development task, you're likely to be making frequent changes to your code, so caching the results is not useful.
- **"persistent": true**: Tells Nrz to keep the task running until you stop it. This key serves as a signal for your terminal UI to treat the task as long-running and interactive. Additionally, it prevents you from accidentally depending on a task that will not exit.
...
Nrz's terminal UI enables a number of features that create a highly interactive experience around your tasks.
...
In some cases, you may want to run a script when the `dev` task is stopped. Nrz is unable to run those teardown scripts when exiting because `nrz` exits when your `dev` tasks exit.

Instead, create a `nrz dev:teardown` script that you run separately after you've exited your primary `nrz dev` task.

## Next steps

Once you have a version of your application that you'd like to deploy, it's time to learn how to configure environment variables in Nrz.
