...
description: Learn how Nrz can help you efficiently complete all the necessary tasks and accelerate your development workflow.
---
...
Nrz speeds up builds, lints, tests, and any other tasks that you need to do in your Continuous Integration pipelines. Through parallelization and [Remote Caching](/repo/docs/core-concepts/remote-caching), Nrz makes your CI dramatically faster.
...
To enable Remote Caching for your CI, setup the environment variables for Nrz to access your Remote Cache.
...
Khulnasoft's built-in CI/CD is automatically connected to your managed Khulnasoft Remote Cache with zero configuration. To retrieve a token for connecting your other CI vendors to Khulnasoft Remote Cache, visit the [Khulnasoft Remote Cache documentation](https://khulnasoft.com/docs/monorepos/remote-caching#use-remote-caching-from-external-ci/cd).

For self-hosted Remote Cache options, visit [Nrz's Remote Cache documentation](/repo/docs/core-concepts/remote-caching#remote-cache-api).
...
Docker is an important part of many deployment pipelines. [Nrz's `prune` subcommand](/repo/docs/reference/prune) helps you ship lightweight images by removing unnecessary dependencies and code from your images.

For more on how to deploy from a Nrz with Docker, visit [the dedicated Docker guide](/repo/docs/guides/tools/docker).
...
CI/CD pipelines are a perfect place to use `--affected`. With `--affected`, Nrz can automatically detect that you're running in GitHub Actions by inspecting environment variables set by GitHub, like `GITHUB_BASE_REF`.

In the context of a PR, this means that Nrz can determine which packages have changed between the PR's base branch and the PR's head branch. This allows you to run only the tasks that are affected by the changes in the PR.
...
Nrz's caching abilities allow you to create fast CI pipelines with minimal complexity. Through [Remote Caching](/repo/docs/core-concepts/remote-caching) and using the `--filter` flag to target packages for builds, Nrz will handle change detection for large monorepos with little overhead.
...
`nrz run` is the most common command you will use when working in your Nrz so it is aliased to `nrz` for convenience. While this is great for working locally, there are other subcommands for `nrz` like [`nrz prune`](/repo/docs/reference/prune) and [`nrz generate`](/repo/docs/reference/generate).
...
If you haven't defined the `env` or `globalEnv` keys for your task, Nrz will not be able to use them when creating hashes. This means your task can hit cache despite being in a different environment.

Check your configuration for the `env` and `globalEnv` keys.

## Next steps

You now have everything you need to ship applications with Nrz. To learn more about specific use cases, [check the Guides](/repo/docs/guides) or [dive deeper into core concepts](/repo/docs/core-concepts).
