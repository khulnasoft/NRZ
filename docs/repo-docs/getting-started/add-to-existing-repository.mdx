...
description: Using Nrz with your existing repository
---
...
Nrz can be incrementally adopted in **any repository, single or multi-package**, to speed up the developer and CI workflows of the repository.
...
A [single-package workspace](https://khulnasoft.com/docs/khulnasoft-platform/glossary#single-package-workspace) is, for example, what you get after running `npx create-next-app` or `npm create vite`. You don't need to do any extra work for Nrz to handle your repo so you can jump to the first step below.

To learn more about Nrz in single-package workspaces, visit [the dedicated guide](/repo/docs/guides/single-package-workspaces).
...
Nrz optimizes your repository using information from your package manager. To declare which package manager you're using, add a [`packageManager`](https://nodejs.org/api/packages.html#packagemanager) field to your root `package.json` if you don't have one already.
...
You can now run the tasks you added to `nrz.json` earlier using Nrz. Using the example tasks from above:
...
You now up and running with Nrz! To learn about more ways you can improve your workflow and get the most out of `nrz`, we recommend checking out the following pages:
