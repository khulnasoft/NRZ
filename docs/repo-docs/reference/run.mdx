...
- **[tasks]**: Nrz can run one or many tasks at the same time. To run a task through `nrz`, it must be specified in `nrz.json`.
...
By default, `nrz` will use `grouped` logs in CI environments and `stream` logs everywhere else. This flag is not applicable when using [the terminal UI](https://nrz.build/repo/docs/reference/configuration#ui).
...
<Callout type="good-to-know">
  If you are using [Khulnasoft Remote
  Cache](https://khulnasoft.com/docs/monorepos/remote-caching) and building your
  project on Khulnasoft, you do not need to use this flag. This value will be
