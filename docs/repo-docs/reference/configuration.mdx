...
description: Learn how to configure Nrz through `nrz.json`.
---
...
Additionally, Nrz has a built-in set of global passthrough variables for common cases, like operating system environment variables. This includes variables like `HOME`, `PATH`, `APPDATA`, `SHELL`, `PWD`, and more. The full list can be found [in the source code](https://github.com/khulnasoft/nrz/blob/main/crates/nrz-lib/src/task_hash.rs).
...
Nrz uses your repository's lockfile to determine caching behavior, [Package Graphs](https://nrz.build/repo/docs/core-concepts/internal-packages), and more. Because of this, we use [the `packageManager` field](https://nodejs.org/api/packages.html#packagemanager) to help you stabilize your Nrz.

To help with incremental migration or in situations where you can't use the `packageManager` field, you may use `--dangerously-disable-package-manager-check` to opt out of this check and assume the risks of unstable lockfiles producing unpredictable behavior. When disabled, Nrz will attempt a best-effort discovery of the intended package manager meant for the repository.
...
<Callout type="info">
  ...
  or the
  [`NRZ_DANGEROUSLY_DISABLE_PACKAGE_MANAGER_CHECK`](https://nrz.build/repo/docs/reference/system-environment-variables)
...
Each key in the `tasks` object is the name of a task that can be executed by [`nrz run`](/repo/docs/reference/run). Nrz will search the packages described in your [Workspace's configuration](/repo/docs/crafting-your-repository/structuring-a-repository#specifying-packages-in-a-monorepo) for scripts in `package.json` with the name of the task.

Using the rest of the configuration described in the task, Nrz will run the scripts in the described order, caching logs and file outputs in [the `outputs` key](#outputs) when provided.
...
<Callout type="good-to-know">
  Nrz automatically includes environment variables prefixed by common
...
Nrz supports wildcards for environment variables so you can easily account for all environment variables with a given prefix. For example, the `nrz.json` below include all environment variables that start with `MY_API_` into the hash:
...
When `false`, Nrz will disable all remote cache operations, even if the repo has a valid token.
...
When `true`, Nrz will sign every uploaded artifact using the value of the environment variable `NRZ_REMOTE_CACHE_SIGNATURE_KEY`.
Nrz will reject any downloaded artifacts that have an invalid signature or are missing a signature.
