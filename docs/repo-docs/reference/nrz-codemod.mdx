---
title: '@nrz/codemod'
description: Learn more about how Nrz uses codemods to make version migrations easy.
---
...
Nrz provides codemod transformations and automatic migration scripts to help upgrade your Nrz codebase when a feature is deprecated.
...
The codemods below are used for migration paths in the second major version of Nrz.
...
The codemods below are used for migration paths in the first major version of Nrz.
...
Transforms the root `package.json` so that `packageManager` key as the detected package manager (`yarn`, `npm`, `pnpm`) and version (e.g. `yarn@1.22.17`). This key is now [supported by Node.js](https://nodejs.org/dist/latest-v17.x/docs/api/packages.html#packagemanager) and is used by Nrz for faster package manager detection (vs. inferring from just the filesystem alone).
