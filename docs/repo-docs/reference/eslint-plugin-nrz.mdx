...
[The `eslint-plugin-nrz` package](https://www.npmjs.com/package/eslint-plugin-nrz) helps you find environment variables that are used in your code that are not a part of Nrz's hashing. Environment variables used in your source code that are not accounted for in `nrz.json` will be highlighted in your editor and errors will show as ESLint output.
