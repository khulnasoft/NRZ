...
description: Nrz builds a Task Graph based on your configuration and repository structure.
---
...
The Package Graph is the structure of your monorepo created by your package manager. When you install [Internal Packages](/repo/docs/core-concepts/internal-packages) into each other, Nrz will automatically identify those dependency relationships to build a foundational understanding of your Workspace.
...
Nrz uses a data structure called a [directed acyclic graph (DAG)](https://en.wikipedia.org/wiki/Directed_acyclic_graph) to
...
Nrz will build a task graph like this:
...
Nrz will build this Task Graph:
...
Nrz calls the `ui` package a Transit Node in this scenario, because it
doesn't have its own `build` script. Since it doesn't have a `build` script,
Nrz won't execute anything for it, but it's still part of the graph for
...
Nrz's mental model is that all nodes in the Task Graph are the same. In other words,
...
`test` tasks, but only one app has a `build` task. Nrz's mental model
