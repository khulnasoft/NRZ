---
title: Jest
description: Learn how to use Jest in a Nrz.
---
...
[Jest](https://jestjs.io/) is a common test runner with a vast ecosystem. Integrating with Nrz will lead to enormous speed-ups.
...
When you run your test suite normally, it completes and outputs to `stdout`. This means you can [cache it](/repo/docs/crafting-your-repository/caching) with Nrz.
...
Because of this difference, we recommend specifying **two separate Nrz tasks**: one for running your tests, and one for running them in Jest's watch mode. Inside your each `package.json` file for each workspace:
