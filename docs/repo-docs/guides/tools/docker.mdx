...
Docker is pretty smart about how it deploys your apps. Just like Nrz, it tries to do as [little work as possible](https://bitjudo.com/blog/2014/03/13/building-efficient-dockerfiles-node-dot-js/).
...
The solution is to prune the inputs to the Dockerfile to only what is strictly necessary. Nrz provides a simple solution - `nrz prune`.
...
Our detailed [`with-docker` example](https://github.com/khulnasoft/nrz/tree/main/examples/with-docker) goes into depth on how to use `prune` to its full potential. Here's the Dockerfile, copied over for convenience.
...
`nrz` will now be able to hit your Remote Cache. To see a Nrz cache hit for a non-cached Docker build image, run a command like this one from your project root:
