...
[Vitest](https://vitest.dev/) is a test runner from the Vite ecosystem. Integrating it with Nrz will lead to enormous speed-ups.
...
Because of this difference, we recommend specifying **two separate Nrz tasks**: one for running your tests, and one for running them in watch mode. Inside your each `package.json` file for each workspace:
