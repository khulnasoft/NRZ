...
description: Learn how to use Nrz in a single-package workspace.
---

import { Callout } from '#/components/callout';
import { PackageManagerTabs, Tab } from '#/components/tabs';

While Nrz is highly effective in [multi-package workspaces](https://khulnasoft.com/docs/khulnasoft-platform/glossary#multi-package-workspace) (commonly referred to as monorepos), it can also be used to make [single-package workspaces](https://khulnasoft.com/docs/khulnasoft-platform/glossary#single-package-workspace) faster.

Nrz's most important features work in single-package workspaces including local and [Remote Caching](/repo/docs/core-concepts/remote-caching) and task parallelization. Features that don't work are ones that don't make sense in the context of a single package, like package tasks (`app#build`).
...
Once installed, you can run `nrz build` and Nrz will run your `build` script from `package.json`. Running `nrz build` again will hit the cache.
...
You can schedule these tasks into one command using Nrz. First, create scripts in your `package.json`:
...
Because Nrz will treat a single-package workspace as one package, it can help to optimize inputs to tasks to make sure unrelated changes don't create cache misses.
