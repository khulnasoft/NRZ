...
description: Learn how to generate code using Nrz.
---
...
experience. With Nrz's code generation, it's easy to generate new source code for packages, modules,
...
```sh title="Terminal"
nrz gen workspace --copy https://github.com/khulnasoft/nrz/tree/main/examples/with-tailwind/packages/tailwind-config
```

<Callout>
  **Note**: When adding from a remote source, Nrz is unable to verify that your repo has all of the required dependencies, and is using the correct package manager. In this case, some manual modifications may be required to get the new workspace working as expected within your repository.
...
Nrz will automatically detect any generator configurations within your repo, and make them available to run from the command line.

<Callout type="info">
  While Nrz Generators are built on top of Plop, they don't require `plop`
  to be installed as a dependency in your repo.
</Callout>

While Nrz understands all Plop configuration options and features, it provides a few additional features to improve the experience of writing
generators within a repo configured with Nrz.
...
To build and run a custom generator, run the following command from anywhere within your monorepo using Nrz.
...
<Callout type="info">
  ...
  package](https://github.com/khulnasoft/nrz/tree/main/packages/nrz-gen) as
