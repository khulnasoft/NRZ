...
That way, when `tsup` is run the outputs can be [cached](/repo/docs/crafting-your-repository/caching) by Nrz.
...
We recommend Changesets because it's intuitive to use, and - just like Nrz - fits with the monorepo tools you're already used to.
...
Linking your publishing flow into Nrz can make organizing your deploy a lot simpler and faster.
...
This means that when you run `publish-packages`, your monorepo gets built, linted, tested and published - and you benefit from all of Nrz's speedups.
