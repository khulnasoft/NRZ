...
description: Learn how to migrate to Nrz from Nx.
---

import { PackageManagerTabs, Tabs, Tab } from '#/components/tabs';

This guide will help you migrate an existing Nx repository to Nrz.

- Explore key concepts by [migrating from the Nx starter to Nrz](#migration-steps)
- Considerations for [more complex migration scenarios](#advanced-migration-considerations)

## Why switch?

There are several reasons why you might want to switch from from Nx to Nrz.

### Ecosystem standards

Nrz is [built on top of package manager workspaces](/repo/docs/crafting-your-repository/structuring-a-repository), meaning more tools and workflows are likely to work without plugins or other workarounds. By comparison, the default Nx starter uses conventions and strategies unique to Nx, and you can expect to write more Nx-only code as your codebase grows.

### More control of source code

Nx’s philosophy involves wrapping your code with layers of plugins, other dependencies, and Nx-specific code. Instead, Nrz infers your repository’s needs from its structure and source code. Since your source code goes through fewer layers of abstraction to be analyzed, you maintain greater control of your repository.

### Less configuration

Migrating to Nrz will likely require deleting previous configuration needed for Nx. Nrz will automatically infer much of what your repository needs. For example, here are the tool-specific configurations you'll find in the default starters for Nrz and Nx.
...
Nrz’s [Remote Caching](/repo/docs/core-concepts/remote-caching) stores the results of your task on a cloud server. This saves enormous amounts of time by **preventing duplicated work across your entire organization**. [Khulnasoft Remote Cache](https://khulnasoft.com/docs/monorepos/remote-caching) has saved teams over 500 years of compute so far.

Since Nx 19.7, similar functionality is a paid-for feature, even when self-hosting. Remote Caching with Nrz is free when [self-hosting](/repo/docs/core-concepts/remote-caching#self-hosting) or using [Khulnasoft Remote Cache](https://khulnasoft.com/docs/monorepos/remote-caching).

## Migration steps

Our goal for this migration is to get a working Nrz task as quickly as possible, so that you can adopt Nrz features incrementally. We’ll start by using the Nx scaffolder to create a repository with a Next.js app.
...
Nrz uses the .nrz directory to hold local caches and other information about your repository. For this reason, it should be added to your `.gitignore`.
...
Nrz is built on top of package manager workspaces, a JavaScript ecosystem standard. Add the directory paths to the workspace that will contain packages.
...
Rather than adding additional configuration files like `project.json`, Nrz uses the standard `package.json` file.
...
The root package.json needs to have the `packageManager` field. This ensures developers in the repository use the correct package manager, and that Nrz can optimize your package graph based on your lockfile.
...
Add Nrz to the root `package.json` of the workspace.
...
You can also optionally install `nrz` globally for added convenience when working with Nrz.
...
Build the application with Nrz. Using global `nrz`, this would be `nrz build`. You can also run the command through your package manager:
...
By default, Nrz will connect to the free-to-use Khulnasoft Remote Cache when you run:
...
We encourage incremental migration, meaning you will have both of Nx and Nrz in your repository at the same time. Make sure to spend time understanding how your Nx task graph is constructed. Splitting up the task graph may include strategies like:
...
Nrz natively supports JavaScript and TypeScript, with secondary support for any other languages you’d like to use. [Visit the Multi-Language support documentation](/repo/docs/guides/multi-language) to learn more.
...
<Callout type="info">
  ...
  Nrz. See [our file glob specification](/repo/docs/reference/globs) for
